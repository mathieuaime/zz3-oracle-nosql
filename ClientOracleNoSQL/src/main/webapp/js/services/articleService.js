	...
	return {
		...
		createArticle: function(id,titre,resume,prix) {
			...
			return deferred.promise;
                ...
                createAEteEcritFromTitre:function(titre,id,titreArticle){
                  var data = {
                      "articleTitre":titre,
                      "idAuteur":id
                  };
			...
			return deferred.promise;
              ...
              createAEteEcrit:function(titre,id,idArticle){
                  var data = {
                      "articleTitre":titre,
                      "idAuteur":id
                  };
		...
		//supprimer tous les auteurs à partir de l'id de l'article
			...
			$http.get('http://localhost:8080/ServerOracleNoSQL/ws/article/'+titre+'/keywordFromTitle').then(function successCallback(response) {
				$log.debug('... réponse du serveur : OK.');
                    ...
                    addKeyword:function(idArticle,keyword,rank){
                  var data = {
                      "idArticle":idArticle,
                      "keyword":keyword,
                      "rank":rank
                ...
                var deferred = $q.defer();
                  $http.post('http://localhost:8080/ServerOracleNoSQL/ws/article/'+idArticle+'/keyword',JSON.stringify(data)).then(function successCallback(response) {
				$log.debug('... réponse du serveur : OK.');
				deferred.resolve(response.data);
			}, function errorCallback(response) {
				$log.debug('... réponse du serveur : erreur.');
