...
public class UniversityWS {
    ...
    @GET
    public RestResponse<University> getAll() {
        List<University> universites = udao.read();
        RestResponse<University> resp = new RestResponse<>(0, universites);
        return resp;
    }

    @Path("{id}")
    @GET
    public RestResponse<University> getUniversite(@PathParam("id") int id) {
        University universite = udao.read(id);
    ...
    @POST
    public RestResponse<University> addUniversite(University universite) {
        int status = udao.create(universite);

        RestResponse<University> resp = new RestResponse<>(status, (status != 0 ? "409" : "201"));
        resp.addObjectList(universite);
        return resp;
    }
    ...
    @Path("{id}")
    @PUT
    public RestResponse<University> updateUniversite(@PathParam("id") int id, University universite) {
        int status = udao.update(id, universite);

        RestResponse<University> resp = new RestResponse<>(status, (status != 0 ? "204" : "200"));
        resp.addObjectList(universite);
        return resp;
    }
    ...
    @Path("{id}")
    @DELETE
    public RestResponse<University> deleteUniversite(@PathParam("id") int id) {
    ...
    public RestResponse<Author> listAuteur(@PathParam("nom") String name) {
        ...
        for (Rattache r : rdao.read(University.MAJOR_KEY, name)) {
